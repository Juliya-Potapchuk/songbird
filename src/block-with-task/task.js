...
export class Task extends React.Component {
  render() {
    ...
    const findIndexObjById = objListBird.findIndex(
      (element) => element.id === correctChoise
    );
    const currentBirdObj = objListBird[findIndexObjById];
    console.log("Правильный ответ:", currentBirdObj.name);
      ...
      return (
        <div className="task-block">
          ...
          <ul className="name-text-ul">
            <li className="name-text-item">
              <Name name={currentBirdObj.name} />
            </li>
            <li className="name-text-item">
              <Audio audio={currentBirdObj.audio} />
            </li>
          </ul>
        </div>
      );
    ...
    if (hasCorrectChoise === "false") {
      return (
        <div className="task-block">
          <Photo
            image={
              "https://birds-quiz.netlify.com/static/media/bird.06a46938.jpg"
            }
          />
          <ul className="name-text-ul">
